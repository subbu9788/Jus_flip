...
import com.juzonce.customer.ui.login.LoginFragment
import com.juzonce.customer.ui.login.LoginFragmentNew
import com.juzonce.customer.ui.register.RegisterFragment
...
class Validator {
    ...
    companion object {
        ...
        fun validationLogin(
            activity: LoginFragmentNew,
            view: View?,
            map: HashMap<String, String>
        ): Boolean {
            if (map.isEmpty()) {
                activity.showSnackView("Enter all fields", view!!)
                return false
            } else if (map[ParamKey.MOBILE_NUMBER].toString().isEmpty()) {
                activity.showSnackView("Enter Mobile Number", view!!)
                return false
            } else if (!validateUser(map[ParamKey.MOBILE_NUMBER].toString())) {
                activity.showSnackView("Enter valid Mobile Number", view!!)
                return false
            }
            return true
        }
